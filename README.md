...
- 任务的组织单元统一称为「分类」；「收集箱」表示任务尚未安排日期，两者彼此独立。
- Popup 默认将快速记录安排到今天，并显示少量今日重点；日期选项可改为明天或收集箱。任务详情使用 Dashboard 右侧抽屉。
...
首次安装扩展时会打开 WorkTodo 使用说明，介绍快速记录、分类和任务推进。更新或重新启动扩展不会再次打开；之后可从工作台「设置」中选择「重新查看使用说明」。

分类统一在工作台「设置」中创建和维护。任务详情的分类选择旁提供直达入口；它会在新标签页打开分类管理，保留当前编辑内容，返回后再次聚焦分类选择即可选中新分类。

任务详情按「基本信息、整理、更多选项」组织；日期和优先级便于快速安排，分类与标签集中管理，低频字段通过折叠行按需查看。
